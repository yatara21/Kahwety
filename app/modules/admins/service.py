...
from app.modules.admins.schemas import AdminCreate, AdminUpdate, AssignPagePermissionsRequest, AdminStatusUpdate
from app.modules.users.repository import UserRepository
from app.modules.users.schemas import UserCreate as UserCreateSchema, UserUpdate as UserUpdateSchema
from app.modules.users.models import User
from app.core.exceptions import NotFoundException, ConflictException, BusinessException, ForbiddenException
...
class AdminService:
    ...
    async def list_admins(
        ...
        admins, total = await self.admin_repository.list_admins(status_enum, search, page, page_size)
        for admin in admins:
            if admin.role == UserRole.SUPER_ADMIN:
                admin.pages = [p.value for p in PagePermission]
            else:
                perms = await self.admin_repository.get_user_permissions(admin.id)
                admin.pages = [p.page.value if hasattr(p.page, "value") else p.page for p in perms]
        return admins, total
    ...
    async def get_admin(self, admin_id: str) -> User:
        ...
        if admin.role == UserRole.SUPER_ADMIN:
            ...
            perms = await self.admin_repository.get_user_permissions(admin_id)
            admin.pages = [p.page.value if hasattr(p.page, "value") else p.page for p in perms]
        return admin
    ...
    async def create_admin(self, admin_create: AdminCreate, creator_role: UserRole) -> User:
        ...
        if admin_create.pages and user.role != UserRole.SUPER_ADMIN:
            await self.admin_repository.replace_permissions(user.id, admin_create.pages)
            user.pages = [p.value if hasattr(p, "value") else p for p in admin_create.pages]
        elif user.role == UserRole.SUPER_ADMIN:
            user.pages = [p.value for p in PagePermission]
        else:
            user.pages = []
    ...
    async def update_admin(self, admin_id: str, admin_update: AdminUpdate, updater_role: UserRole, updater_id: str) -> User:
        ...
        if updater_role != UserRole.SUPER_ADMIN and updater_id != admin_id:
            raise ForbiddenException("Only SUPER_ADMIN can manage other admins")
        ...
        # Prevent removing the final active SUPER_ADMIN
        ...
        if admin_update.password:
            ...
            updated_user.password_hash = get_password_hash(admin_update.password)
            await self.session.flush()
            await self.session.refresh(updated_user)

        # Update pages if provided
        if admin_update.pages is not None and updated_user.role != UserRole.SUPER_ADMIN:
            await self.admin_repository.replace_permissions(admin_id, admin_update.pages)
            updated_user.pages = [p.value if hasattr(p, "value") else p for p in admin_update.pages]
        elif updated_user.role == UserRole.SUPER_ADMIN:
            updated_user.pages = [p.value for p in PagePermission]
        else:
            perms = await self.admin_repository.get_user_permissions(admin_id)
            updated_user.pages = [p.page.value if hasattr(p.page, "value") else p.page for p in perms]
        
        return updated_user

    async def update_admin_status(self, admin_id: str, status: UserStatus, updater_role: UserRole, updater_id: str) -> User:
        if updater_role != UserRole.SUPER_ADMIN:
            raise ForbiddenException("Only SUPER_ADMIN can change admin status")
            
        admin = await self.admin_repository.get_admin(admin_id)
        if not admin:
            raise NotFoundException("Admin not found")
            
        if admin_id == updater_id and status in {UserStatus.INACTIVE, UserStatus.SUSPENDED}:
            raise BusinessException("Cannot deactivate or suspend yourself")
            
        if admin.role == UserRole.SUPER_ADMIN and status != UserStatus.ACTIVE:
            active_super_admins = await self.admin_repository.count_super_admins_status(UserStatus.ACTIVE)
            if active_super_admins <= 1:
                raise BusinessException("Cannot deactivate the last active SUPER_ADMIN")
                
        user_update = UserUpdateSchema(status=status)
        return await self.user_repository.update(admin, user_update)
    ...
    async def get_admin_permissions(self, admin_id: str) -> List[PagePermission]:
        ...
        return [PagePermission(p.page) for p in permissions]
    ...
    def list_available_permissions(self) -> List[str]:
        return [p.value for p in PagePermission]
